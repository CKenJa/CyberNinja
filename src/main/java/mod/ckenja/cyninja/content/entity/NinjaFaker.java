package mod.ckenja.cyninja.content.entity;
...
public class NinjaFaker extends PathfinderMob {
    ...
    public void tick() {
        ...
        if (owner != null && !this.level().isClientSide() && new EquipmentCondition(EquipmentSlotGroup.ARMOR, Items.AMETHYST_SHARD).test(owner)) {//TODO 発動しているかどうかに条件を変える
            ActionAttachment data = this.getData(ModAttachments.ACTION);
            Holder<Action> ownerAction = owner.getData(ModAttachments.ACTION).getCurrentAction();
