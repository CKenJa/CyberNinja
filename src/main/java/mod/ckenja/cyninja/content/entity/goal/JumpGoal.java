package mod.ckenja.cyninja.content.entity.goal;
...
public class JumpGoal extends TimeConditionGoal {
    ...
    public void tick() {
        super.tick();
        ActionAttachment data = mob.getData(ModAttachments.ACTION);
        if (this.tick == 3) {
            data.syncAction(mob, NinjaActions.AIR_JUMP);
        }
        if (this.tick == 7 && this.mob.isHolding(ModItems.KATANA.asItem())) {
            data.syncAction(mob, NinjaActions.SPIN);
        }
    }
