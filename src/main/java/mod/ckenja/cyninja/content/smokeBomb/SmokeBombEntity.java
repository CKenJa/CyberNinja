package mod.ckenja.cyninja.content.smokeBomb;
...
public class SmokeBombEntity extends ThrowableItemProjectile {
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            ...
            //TODO 発動しているかどうかに条件を変える
            if (this.getOwner() instanceof LivingEntity attacker && new EquipmentCondition(EquipmentSlotGroup.HAND, ModItemTags.SMOKE_BOMB, Items.LAPIS_LAZULI).test(attacker)) {
                entities.forEach(entity -> {
                    if (entity instanceof LivingEntity living)
            ...
            if (this.getOwner() instanceof LivingEntity attacker && new EquipmentCondition(EquipmentSlotGroup.HAND, ModItemTags.SMOKE_BOMB, Items.EMERALD).test(attacker)) {
            ...
            smoking_time++;
            if (10 < smoking_time) {
                if (this.getOwner() instanceof LivingEntity attacker && new EquipmentCondition(EquipmentSlotGroup.HAND, ModItemTags.SMOKE_BOMB, Items.QUARTZ).test(attacker)) {
